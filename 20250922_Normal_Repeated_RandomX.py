...
def generate_dataset(n_total, dist_type='normal', outlier_ratio=0.05, outlier_magnitude=10, error_std=0.5):
    """
    ...
    if dist_type == 'normal':
        # Generates data following a normal distribution.
...
def main():
    ...
    """
    print("--- Simulation with Normal Distribution Dataset ---")
    
    # 1. Generate the full dataset once
    x_total, y_total = generate_dataset(
        N_TOTAL, 
        dist_type='normal', # Changed to 'normal'
    ...
    for method, lines in results.items():
        ...
        # Updated title to specify 'Normal Data'
        plt.title(f'{plot_titles[method]} on Normal Data (100 Trials)', fontsize=16)
