...
def run_simulation_and_plot(x_total, y_total, dist_name):
    ...
    ERROR_VAR_RATIO = 0.9 # Set the error variance ratio (delta)
    ...
    for i in range(N_TRIALS):
        ...
        # --- Deming Regression with specified error variance ratio ---
        linear_model = Model(lambda p, x: p[0] * x + p[1])
        # The ratio of weights (wd/we) is the error variance ratio. we=1/delta if wd=1.
